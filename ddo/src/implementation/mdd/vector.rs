...
use std::{collections::{hash_map::Entry, HashSet}, fmt::Debug, fs, hash::{BuildHasherDefault, Hash}, sync::Arc};

use derive_builder::Builder;
// use derive_builder::Builder;
use fxhash::{FxHashMap, FxHashSet};

use crate::{NodeFlags, Decision, CutsetType, CompilationInput, Completion, Reason, CompilationType, CompilationStrategy, Problem, LAST_EXACT_LAYER, DecisionDiagram, SubProblem, FRONTIER, Solution, DominanceCheckResult};
...
macro_rules! get {
    (    node     $id:expr, $dd:expr) => {&    $dd.nodes   [$id.0][$id.1]};
    (mut node     $id:expr, $dd:expr) => {&mut $dd.nodes   [$id.0][$id.1]};
    ...
    (    layer    $id:expr, $dd:expr) => {&    $dd.nodes  [$id.0]};
    (mut layer    $id:expr, $dd:expr) => {&mut $dd.nodes  [$id.0]};
}
    ...
    T: Eq + PartialEq + Hash + Clone,
{
    ...
    fn refine(
        &mut self,
        input: &CompilationInput<Self::State>,
    ...
    T: Eq + PartialEq + Hash + Clone,
{
        ...
        if input.comp_type == CompilationType::Relaxed || self.is_exact {
            ...
            if let Some(best_exact_node) = self.best_exact_node {
                ...
                for id in &self.final_l {
                    if (CUTSET_TYPE == LAST_EXACT_LAYER && self.is_exact) || (CUTSET_TYPE == FRONTIER && get!(node id, self).flags.is_exact()) {
    ...
    fn _filter_with_dominance_edge_based(&mut self, input: &CompilationInput<T>, curr_l: &mut Vec<NodeId>) {
        let mut curr_in_edges = vec![];
        for node_id in curr_l.iter(){
            curr_in_edges.extend(get!(node node_id, self).incoming.iter()
                            .filter_map(|x| {
                                (!get!(node get!(edge x, self).from, self).flags.is_deleted() && 
                                !get!(node get!(edge x, self).from, self).flags.is_pruned_by_dominance() && 
                                !get!(node get!(edge x, self).from, self).flags.is_pruned_by_cache() && 
                                !input.problem.filter(&get!(node get!(edge x, self).from, self).state, &get!(edge x, self).decision)).then_some(*x) 
                            })
                            .collect::<Vec<_>>());
        }
        
        curr_in_edges.sort_unstable_by(|a,b| input.dominance.cmp(get!(edge a, self).state.as_ref(), 
        ...
        curr_in_edges.retain(|id| {
            ...
            let mut src_outgoing = get!(node src_id, self).outgoing.iter().map(|x|*x).collect::<Vec<_>>();
            let mut dst_incoming = get!(node dst_id, self).incoming.iter().map(|x|*x).collect::<Vec<_>>();
            
            if get!(node src_id,self).flags.is_exact() {
                let DominanceCheckResult { dominated, threshold } = input.dominance.is_dominated_or_insert(get!(edge id, self).state.clone(), 
                ...
                if dominated {
                    ...
                    if src_outgoing.is_empty(){
                        ...
                        get!(mut node src_id, self).flags.set_pruned_by_dominance(true);
                    }
                    if dst_incoming.is_empty(){
                        // set node as deleted and remove from layer                        
                        get!(mut node dst_id, self).flags.set_pruned_by_dominance(true);
    ...
    fn _filter_constraints(&mut self, input: &CompilationInput<T>, curr_l: &mut Vec<NodeId>) {
        for node_id in curr_l.iter() {
            ...
            let inbound = get!(node node_id, self).incoming.iter()
                                                        .filter_map(|x| {
                                                            (!get!(node get!(edge x, self).from, self).flags.is_deleted() && 
                                                            !get!(node get!(edge x, self).from, self).flags.is_pruned_by_dominance() && 
                                                            !get!(node get!(edge x, self).from, self).flags.is_pruned_by_cache() && 
                                                        ...
                                                        .collect::<Vec<_>>();
            let inbound_unfiltered = get!(node node_id, self).incoming.iter()
            .collect::<Vec<_>>();
            ...
            match input.comp_type {
                ...
                _ => {
                    ...
                    while curr_l.len() < input.max_width {
                        ...
                        if fully_split {break;} // nothing to stretch
                    }
                    if fully_split && 
    ...
    fn _split(
        ...
        for node_id in &to_split{            
            inbound_edges.extend(get!(node node_id, self).incoming.iter()
                                            .map(|x| (x.0, 
        ...
        let split_state_edges = input.problem.split_edges(&mut inbound_edges.into_iter(),how_many);

        let split_states: Vec<(Arc<T>, Vec<usize>)>  = split_state_edges.into_iter().map(|cluster| {
        ...
        let mut new_nodes = self._redirect_edges_after_split(input,&split_states,LayerId(curr_layer_id));
    ...
    fn _merge_states_from_incoming_edges (
        ...
        let merged = Arc::new(
            input
                .relaxation
                .merge(&mut inbound_edges.iter().map(|x| get!(edge EdgeId(*x), self).state.as_ref())),
        );
        merged
    }
    ...
    fn _redirect_edges_after_split(&mut self,
        ...
        curr_layer_id: LayerId,) -> Vec<NodeId> {
        ...
        for (state, incoming_edges) in split_states{
            ...
            curr_layer.push(Node {
                ...
                theta: None,
                // flags: NodeFlags::new_relaxed(),
                flags: if incoming_edges.len() == 1 {NodeFlags::new_exact()} else {NodeFlags::new_relaxed()},
                depth,
            });
            ...
            for edge_id in incoming_edges{
                ...
                outbound_edges.extend(get!(node outbound_node, self).outgoing.iter().collect::<Vec<_>>());
            }
        ...
        fn _redirect_incoming_edges(&mut self, input: &CompilationInput<T>, edges_to_append: &Vec<usize>, state: &&Arc<T>, split_id: NodeId, _outbound_edges: &FxHashSet<EdgeId>) {
            for edge_id in edges_to_append {
                // update edge state
                let from_node = get!(node get!(edge EdgeId(*edge_id), self).from, self);
                let new_state = Arc::new(input.problem.transition(from_node.state.as_ref(), get!(edge EdgeId(*edge_id), self).decision));
        ...
        //TODO these are not checked for duplications? What if there's already an edge like tis to that destination node?
        fn _redirect_outgoing_edges(&mut self, input: &CompilationInput<T>, outbound_edges: &FxHashSet<EdgeId>, state: &&Arc<T>, split_id: NodeId) -> Vec<NodeId> {
        ...
        fn update_node(&mut self, input: &CompilationInput<T>, node_id: NodeId){
            ...
            let inbound_edges = get!(node node_id, self).incoming.iter().map(|x| x.0).collect();
