...
use crate::{
    ...
    DefaultVectorLEL, DominanceChecker, EmptyCache, Fringe, Problem, Reason, Relaxation, Solution,
    ...
    C: Cache<State = State> + Default,
{
    ...
    fn initialize(&mut self) -> Result<Completion, Reason> {
        ...
        // always start w/ small width diagram
        let width = 2;
...
impl<'a, State, D, C> Solver
    for IncrementalSolver<'a, State, D, C>
where
    ...
    C: Cache<State = State> + Default,
{
    ...
    fn maximize(&mut self) -> Completion {
        ...
        let root = self.root_node();
        // this width should be greater than starting width of 10 otherwise nothing happens
